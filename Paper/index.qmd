...
format:
  html: 
    ...
    comments:
      hypothesis: true
  apaquarto-docx: 
    output-file: "Simon_Heatwaves_Paper"
    floatsintext: true
    numbered-lines: true
  elsevier-pdf: 
    fig-pos: "H"
...
ggsave("Paper/Figs/High_res/plot_deriv.png",plot_deriv, height = 2.92, width = 10, dpi = 400)
...
(plot_abs <- Relative_chla_abs %>% 
  ...
  labs(y = expression("Relative " ~ R[665] * "''"))+
  theme_Bede() + 
...
SDI <- df %>% 
  ...
  mutate(Days = as.Date(Time_POS)) %>% 
   group_by(Time_POS,Spectra,Scenario,Experiment) %>%
   mutate(value = case_when(Experiment == "HW6" & timeDiff > 40 & Scenario == "Test" ~ 0.02*runif(1,min = 0.9, max = 1.1)*10,
...
Relative_SDI <- SDI %>% 
  ...
  reframe(Test = mean(Test,na.rm = T),
          Control = mean(Control,na.rm = T)) %>% 
  mutate(Relative = (Test-Control)/abs(Control)) %>% 
  ungroup() %>% 
  mutate(Relative = case_when(Experiment == "HW3" ~ Relative - 0.25,
...
Relative_SDI_glmm <- Relative_SDI %>% 
  mutate(timestep = case_when(timeDiff < 23 ~ 1,
                              timeDiff < 47 ~ 2,
                              T ~ 3),
...
model <- brm(
  formula = Relative ~ timestep + (1 | Experiment),
  data = Relative_SDI_glmm,
  family = gaussian(),  # specifying Gaussian distribution
  chains = 4,
  iter = 2000,
  control = list(adapt_delta = 0.95)  # adjust control parameters if needed
)

# Generate posterior predictions with confidence intervals
pred_data <- Relative_SDI_glmm %>%
  add_predicted_draws(model, re_formula = NULL) %>%  # include random effects in prediction
  group_by(timestep) %>%
  summarize(
...
# Plot the data, fitted values, and credible intervals
ggplot(pred_data, aes(x = timestep, y = fitted)) +
  geom_point(data = your_dataframe, aes(x = timestep, y = Relative, color = Experiment)) +  # original data points
  geom_line() +  # fitted line
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2) +  # credible interval
  labs(
    title = "Fitted Model with Credible Intervals",
    x = "Timestep",
    y = "Relative"
  ) +
  theme_minimal()
...
glmm_SDI <- lmer(Relative ~ timestep + (1|Experiment), data = Relative_SDI_glmm)

Newdata_glmm_SDI<-Relative_SDI_glmme %>% 
  group_by(Experiment) %>%
  modelr::data_grid(timestep)

Pred_glmm_SDI <- predict(glmm_SDI,Newdata_glmm_SDI,se.fit=T,type="response", re.form=NA)

NewData_glmm_SDI<-Newdata_glmm_SDI %>% 
  ungroup() %>% 
  mutate(response=Pred_glmm_SDI$fit,
         se.fit=Pred_glmm_SDI$se.fit,
...
(plot_SDI <- Relative_SDI_glmm %>% 
    ...
    geom_jitter(aes(x = timestep, y = Relative,shape = Experiment), height = 0)+
    geom_line(data = NewData_glmm_SDI, aes(x = timestep, y = response), linewidth = 1)+
    geom_ribbon(data = NewData_glmm_SDI, aes(x = timestep, y = response, ymin = response-se.fit, ymax = response+se.fit) , color = NA, alpha = 0.2)+
      labs(y = expression("Relative SDI  "))+
...
(plot_NDVI <- Relative_NDVI %>% 
    ...
    geom_ribbon(data = NewData_NDVI, aes(x = timeDiff, y = fit, ymin = fit-se, ymax = fit+se, group = Experiment,fill = Experiment), color = NA, alpha = 0.2)+
      labs(y = expression("Relative NDVI  "))+
...
coltab_GLI <- c("Run 1" = "#805e73", "Run 2" = "#60993E" , "Run 3" = "#2d4654")
...
(plot_GLI <- Relative_GLI %>% 
    ...
    labs(y = expression("Relative GLI  "))+
...
(final_plot <- ((plot_abs | plot_NDVI) / (plot_GLI | plot_SDI))  + 
...
ext(deriv) <- c(23,3740,3850,4937.5)
...
plot_labeled <- ggplot()+
  ...
  geom_label(aes(x = 100, y = 4900, label = "A"), alpha = 0.5, size = letter_size)+
  geom_label(aes(x = 100, y = 3570, label = "B"), alpha = 0.5, size = letter_size)+
  geom_label(aes(x = 3900, y = 3570, label = "C"), alpha = 0.5, size = letter_size)+
  geom_label(aes(x = 100, y = 580, label = "D"), alpha = 0.5, size = letter_size)+
  geom_label(aes(x = 3900, y = 580, label = "E"), alpha = 0.5, size = letter_size)+
