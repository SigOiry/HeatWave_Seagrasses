...
**TRY1 :** 
$$
\text{SDI} = \frac{\left( R(705) - R(665) \right) - \left( R(560) - R(450) \right)}{R(705) + R(665) + R(560) + R(450)}
$$ {#eq-SDI}
...
stk <- c(NDVI_before,NDVI_after,GLI_before,GLI_after) %>% 
  crop(seagrass_mask,mask = T) 

writeRaster(stk,"Data/Sentinel2/Before_after_indices_meadowCropped.tif")
...
rast_darkening <- stk_litto  %>% 
  dplyr::select(c(x,y,diff_GLI)) %>% 
  rast(type = "xyz", crs = "EPSG:32630") 

writeRaster(rast_darkening, "Output/diff_gli.tif", overwrite = T)
  
dark_seagrass <- rast_darkening > 0.18
...
write_sf(sf_darkseagrass,"Output/darkening_patches.shp")
...
gam_model <- mgcv::gam(percentage_GLI ~ s(emersion_per_day,
                                          k=6
                                          ), 
...
NewData<-expand_grid(emersion_per_day=unique(stk_all_df_gam$emersion_per_day))
...
(plot_GAM <- ggplot(NewData)+
  ...
  ylab("Relative change of the GLI (%)")+
  xlab("Emerged time per day (hour)")+
  geom_point(data = stk_all_df_gam , aes(x = emersion_per_day , y = percentage_GLI), alpha = 0.05)+
  theme_Bede()+
  xlim(c(10,15))+
...
sf_darkseagrass <- read_sf("../Output/darkening_patches.shp") %>% 
  mutate(area = st_area(geometry),
...
darkenened <- rast_all %>% 
  dplyr::filter(diff_GLI > 0.18)

Percent_above_bathy <- darkenened %>% 
  mutate(Above = case_when(bathy > 3.9 ~ T,
                           T ~ F)) %>% 
  group_by(Above) %>% 
  reframe(nobs = n()) %>% 
...
Additionally, analysis of bathymetry data to determine seagrass emersion time shows a clear relationship between the time seagrass is exposed to air and its darkening. During the heatwave, no significant darkening occurred with daily exposure below 13 hours. However, when exposure reached 13 hours or more per day, seagrasses began to darken, peaking at around 14.5 hours of daily exposure [@fig-GAM_Emersion].

```{r Fig GAM_Emersion}
#| cache: false
#| echo: false
#| warning: false
#| fig-cap: "Relative change in the Green Leaf Index (GLI) before and during heatwave events as a function of the daily emergence time of seagrasses. The line represents a Generalized Linear Model (GLM) prediction, and the shaded area indicates the standard error. Shaded points represent raw data, with each point corresponding to a single pixel."
