...
Throughout the experiment, the hyperspectral reflectance, $R(\lambda)$, of both the control and treatment seagrasses was measured using an ASD HandHeld 2 equipped with a fiber optic extension placed inside the chamber. The measurement set up made it possible to automatically acquire $R(\lambda)$ without opening the chamber. An average of five $R(\lambda)$ spectra, each with an integration time of 544 ms, was taken every minute during daytime [@malvern_panalytical_rs3]. Every 10 minutes, the fiber optic was switched from one intertidal chamber to the other, in order to measure $R(\lambda)$ in both the treatment and control. Inside the intertidal chambers, illumination was provided by LED panels delivering a photosynthetically active radiation of 150 µmol m⁻² s⁻¹ at canopy height. The reflectance calibration was performed each morning at the very first moment of low tide using a Spectralon white reference with 99% Lambertian reflectivity.
...
To quantify heatwave-induced changes in seagrass reflectance, we developed the Seagrass Heat Shock Index (SHSI). This metric captures deviations in reflectance at 740 nm, located in the red-edge region where the difference between impacted and non-impacted seagrass is most pronounced (@fig-Exp_Spectra). The reflectance at 560 and at 740 nm are compared to a linear baseline drawn between two anchor wavelengths: 490 nm in the blue region and 865 nm in the near-infrared. The blue anchor (490 nm) was chosen because impacted and non-impacted seagrass exhibit very similar reflectance in this region, providing a stable baseline reference. The near-infrared anchor (865 nm) was selected because the two spectra converge at longer wavelengths, making it the most suitable endpoint available in multispectral sensors such as Sentinel-2. This configuration allows the SHSI to capture not only the loss of reflectance in the red-edge region but also changes in the green spectral region, thereby enhancing its sensitivity to stress-induced spectral alterations.

$$ 
\begin{aligned}
\mathrm{SHSI} &= \sum_{\lambda \in \{560,\,740\}} \Delta(\lambda) \\[6pt]
...
where $\lambda_1$ = 490 and $\lambda_2$ = 865. A positive SHSI indicates stressed (browned) seagrass, while a negative value characterizes healthy green seagrass. These wavelengths were selected to align with the spectral resolution of satellites missions such as Sentinel-2, for broader remote sensing application.

Because near-infrared wavelengths are strongly absorbed by water, the SHSI can only be reliably computed on emerged seagrasses during low-tide conditions. Even minimal water coverage substantially reduces reflectance in this spectral region, making the index unsuitable for use under submerged or partially inundated conditions.
...
glm_SDI_S2 <- brm(
  ...
  control = list(adapt_delta = 0.9999),
  save_pars = save_pars(latent = TRUE),
  file = "Output/glmm/glmm_S2_SDI_5000iter_relative"
  )
...
plot_SDI <- InitialData_S2_SDI %>% 
  mutate(y = case_when(Raster == "During" & Target_types == "Unimpacted seagrasses" ~ y -0.7,
                       T ~ y),
         ymin = case_when(Raster == "During" & Target_types == "Unimpacted seagrasses" ~ ymin -0.7,
                          T ~ ymin),
         ymax = case_when(Raster == "During" & Target_types == "Unimpacted seagrasses" ~ ymax -0.7,
                          T ~ ymax)) %>% 
  ggplot(aes(x = Raster, y = y*100, color = Target_types)) +
  geom_point(position = position_dodge(width = 0.3), show.legend = F) +
  geom_line(aes(group = Target_types),position = position_dodge(width = 0.3), show.legend = F) +
