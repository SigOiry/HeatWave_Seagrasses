...
sst_data <- mhw_segments %>%
  filter(t > as.Date("2021-08-01") & t < as.Date("2021-10-20")) %>%
  mutate(variable = "SST")
...
Combined_plot <- ggplot() +
  ...
  geom_segment(
    data = sst_data,
    aes(x = t, y = temp, xend = t_lead, yend = temp_lead, linetype = linetype,
    color = "line_water"),
  ) +
  geom_flame(
    data = sst_data,
  ...
  scale_color_manual(values = tabcol,
                     name = NULL,
                     label = c("Air Temperature","90th percentile of Air Temperature", "SST","90th percentile of SST"))+
...
ggsave("Paper/Figs/Sat_SST_Air/Temp_plot.jpg",Combined_plot,height = 1058*2, width = 1920 *2, unit = "px")

```

```{r Combine Temp_plot and S2}
#| cache: true
#| echo: false
#| eval: false
#| warning: false
...
Temp <- rast("Paper/Figs/Sat_SST_Air/Temp_plot.jpg")
ext(Temp) <- c(0,3840,0,2116)

before <- rast("Data/imgs/S2_BeforeHeatwave.png")
ext(before) <- c(-251*3.407407,0,100,621*3.407407)

after <- rast("Data/imgs/S2_afterHeatwave.png")
ext(after) <- c(3840, 3840 + (251*3.407407),100,621*3.407407)
...
ggsave("Paper/Figs/High_res/SST_Air_S2.png",all_plot, width = 10, height = 3.81 , dpi = 800, background = "white")
...
#| echo: false
#| warning: false
#| fig-cap: Visual inspection of the meadow before (left) and after (right) of both marine heatwave and atmospherical heatwave (center). In the center plot, colored lines show 90th percentile of SST (blue) and air temperature (red), while colored polygons represent Marine heatwave (blue) and atmospherical heatwave (red). Darklines represent air temperature (top) and SST (bottom). 
#| label: fig-Temp_plot_S2
#| fig-width: 10
#| fig-height: 3.81
#| out-width: "100%"

knitr::include_graphics("Figs/Low_res/SST_Air_S2.png")
```
...
result_cleaned_NDVI <- result_cleaned %>% 
  ...
  mutate(date = case_when(Raster == "After" ~ ("2021-09-06"),
                          T ~ ("2021-09-01")),
         Desc = case_when(Target_types == "Bleached_seagrass" ~ "Seagrass seen as darkened on the field", 
                          T ~ "Seagrass seen as green on the field")
         )



coltab = c("2021-09-01" = "#1C86EE", "2021-09-06" = "goldenrod")


plot <-result_cleaned_NDVI %>% 
  # dplyr::filter(Desc == "Seagrass seen as darkened on the field") %>% 
ggplot()+
    geom_line(aes(x = Wavelength, y = mean, color = date),linewidth = 1)+
    geom_ribbon(aes(x = Wavelength, y = mean, ymin = mean - sd, ymax = mean + sd, color = NULL, fill = date), alpha = 0.1)+
      facet_wrap(~ Desc)+
  scale_color_manual(values = coltab,
                     name = "Date of S2 images:")+
  scale_fill_manual(values = coltab,
                    name = "Date of S2 images:")+
  ylab("Standardised Sentinel-2 Reflectance")+
    theme_Bede()+
  theme(legend.position = c(0.9,0.1),
        plot.background = element_rect(fill = "white"))

ggsave("Paper/Figs/High_res/S2_Hugo.png", plot, width=1172 * 3 ,height=589 *3, unit = "px")
...
#| echo: false
#| warning: false
#| fig-cap: "Comparison of S2 reflectance before and after the heatwaves event for GCP classified has darkened seagrass (left) and green seagrass (right) during the fieldtrip of the 10th of September 2021."
#| label: fig-S2_comparison
#| fig-width: 10
#| fig-height: 5
#| out-width: "100%"

knitr::include_graphics("Figs/High_res/S2_Hugo.png")
```
