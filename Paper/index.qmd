...
Monitoring the second derivative at 665 nm ($R''_{665 \, \text{nm}}$) indicated that, at the start of the experiment (day 1) $R''_{665 \, \text{nm}}$ for the Treatment group was on average `r round(df_model_hyperspectral %>%  dplyr::filter(days == 1 & variable == "abs_chla") %>% pull(y) *100)` higher than that of the Control. However, by day 2, $R''_{665 \, \text{nm}}$ for the Treatment showed a rapid decrease of approximately `r round(df_model_hyperspectral %>%  dplyr::filter(days == 2 & variable == "abs_chla") %>% pull(y) *100)` %, eventually reaching a total decline of `r round(df_model_hyperspectral %>%  dplyr::filter(days == 3 & variable == "abs_chla") %>% pull(y) *100)` % by day 3 (@fig-Exp_Spectral_indices B)

Monitoring the Normalized Difference Vegetation Index (NDVI) revealed that, at the beginning of the experiment (day 1), the NDVI for the Treatment group was on average `r abs(round(df_model_hyperspectral %>% dplyr::filter(days == 1 & variable == "NDVI") %>% pull(y) *100))` % lower than that of the Control. By day 2, the NDVI for the Treatment exhibited a rapid decline of approximately `r round(df_model_hyperspectral %>% dplyr::filter(days == 2 & variable == "NDVI") %>% pull(y) *100)` %, eventually reaching a cumulative decrease of `r round(df_model_hyperspectral %>% dplyr::filter(days == 3 & variable == "NDVI") %>% pull(y) *100)` % by day 3 (Fig. Exp_Spectral_indices C).

Monitoring the Green Leaf Index (GLI) showed that, at the start of the experiment (day 1), the GLI for the Treatment group was, on average, `r round(df_model_hyperspectral %>% dplyr::filter(days == 1 & variable == "GLI") %>% pull(y) *100)` % higher than that of the Control. However, by day 2, the GLI for the Treatment exhibited a rapid decrease of approximately `r round(df_model_hyperspectral %>% dplyr::filter(days == 2 & variable == "GLI") %>% pull(y) *100)` %, ultimately reaching a cumulative reduction of `r round(df_model_hyperspectral %>% dplyr::filter(days == 3 & variable == "GLI") %>% pull(y) *100)` % by day 3 (Fig. Exp_Spectral_indices D).

Monitoring the Simple Difference Index (SDI) revealed that, at the start of the experiment (day 1), the SDI for the Treatment group was on average `r round(df_model_hyperspectral %>% dplyr::filter(days == 1 & variable == "SDI") %>% pull(y) *100)` % higher than that of the Control. By day 2, the SDI for the Treatment exhibited a rapid decrease of approximately `r round(df_model_hyperspectral %>% dplyr::filter(days == 2 & variable == "SDI") %>% pull(y) *100)` %, eventually reaching a cumulative decline of `r round(df_model_hyperspectral %>% dplyr::filter(days == 3 & variable == "SDI") %>% pull(y) *100)` % by day 3 (Fig. Exp_Spectral_indices E).
...
Using Sentinel-2 data, the remote sensing derived metrics, the NDVI, the Seagrass Cover (SC), the GLI and the SDI estimated for pixels unaffected by the heatwave showed minimal changes for all metrics of `r round(df_indices_S2 %>% dplyr::filter(Raster == "During" & Target_types == "Unimpacted seagrasses" & Indices == "NDVI") %>% pull(y) *100)` %, `r round(df_indices_S2 %>% dplyr::filter(Raster == "During" & Target_types == "Unimpacted seagrasses" & Indices == "SPC") %>% pull(y) *100)` %, `r round(df_indices_S2 %>% dplyr::filter(Raster == "During" & Target_types == "Unimpacted seagrasses" & Indices == "GLI") %>% pull(y) *100)` and `r round(df_indices_S2 %>% dplyr::filter(Raster == "During" & Target_types == "Unimpacted seagrasses" & Indices == "SDI") %>% pull(y) *100)` %, respectively between the 1st of September and the 6th of September (@fig-NDVI_GLI_SPC). In contrast, seagrass impacted by the heatwave exhibited a significant changes in all metrics. Because the SC is linearly linked with the NDVI, both metric drop of `r mean(c(round(df_indices_S2 %>% dplyr::filter(Raster == "During" & Target_types == "Impacted seagrasses" & Indices == "NDVI") %>% pull(y) *100), round(df_indices_S2 %>% dplyr::filter(Raster == "During" & Target_types == "Impacted seagrasses" & Indices == "SPC") %>% pull(y) *100)))` %, however the drop in SC were not due to a loss of density of seagrass on the field (@fig-QuiberonImg). The GLI drops of about `r round(df_indices_S2 %>% dplyr::filter(Raster == "During" & Target_types == "Impacted seagrasses" & Indices == "GLI") %>% pull(y) *100)` % at the 6th of September. The SDI were the most sensitive metric to the darkening of seagrasses, showing an increase of `r round(df_indices_S2 %>% dplyr::filter(Raster == "During" & Target_types == "Impacted seagrasses" & Indices == "SDI") %>% pull(y) *100)` % during heatwave exposure.

In this case, the NDVI dropped by approximately 34%, from a median of 0.61 to 0.40 (@fig-NDVI_GLI_SPC B). Although the heatwave did not affect the observed seagrass density in the field, the drop in NDVI led to an underestimation of satellite-based SPC, from 83 to 48% (@fig-NDVI_GLI_SPC D). The GLI, the index that reflects the greenness of leaves, was the most affected by the event, with an important reduction from a median of 0.23 to 0.03 during the event (@fig-NDVI_GLI_SPC F). Interestingly, when comparing the initial seagrass density of both targets (i.e., seagrass density before the event) pixels affected by the heatwave (median SPC = 85%; @fig-NDVI_GLI_SPC D green) were denser than the those remained unchanged (median SPC = 65%; @fig-NDVI_GLI_SPC C green).
