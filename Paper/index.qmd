...
With a maximum deviation of `r round(df_model_hyperspectral |> dplyr::filter(days == 3 & variable == "SDI") |> dplyr::pull(y) *100)` %, SHSI emerges as the most sensitive index for detecting seagrass browning. Consequently, only this index will be considered for the next steps of this study.
...
## Heatwave of September 2021 in Quiberon, South Brittany
...
The Sentinel-2 images analyzed in this study are dated from the 1st of September 2021 and the 6th of September 2021 (@fig-S2_comparison A and C, respectively). The Atmospheric Heat Wave (AHW) started on the 4th of September and lasted until the 7th of September, while the Marine Heat Wave (MHW) started on the 3rd of September and ended on the 8th of September 2021 (@fig-S2_comparison B). Both air and water temperatures experienced a sharp increase. Between September 1st and 6th, the daily average air temperature rose from `r df_Temp |> dplyr::filter(variable == "Air" & t == "2021-09-01") |> dplyr::pull(temp)` to `r df_Temp |> dplyr::filter(variable == "Air" & t == "2021-09-07") |> dplyr::pull(temp)` while the daily average water temperature increased from `r round(df_Temp |> dplyr::filter(variable == "SST" & t == "2021-09-01") |> dplyr::pull(temp),1)` to `r round(df_Temp |> dplyr::filter(variable == "SST" & t == "2021-09-07") |> dplyr::pull(temp),1)` °C. During this period, the 90th percentile temperatures were `r round(df_Temp |> dplyr::filter(variable == "Air" & t == "2021-09-01") |> dplyr::pull(thresh),1)` °C for air and `r round(df_Temp |> dplyr::filter(variable == "SST" & t == "2021-09-01") |> dplyr::pull(thresh),1)` °C for water. The air temperature anomaly of  `r df_Temp |> dplyr::filter(variable == "Air" & t == "2021-09-07") |> dplyr::pull(temp) - round(df_Temp |> dplyr::filter(variable == "Air" & t == "2021-09-01") |> dplyr::pull(seas),1)` °C classified the atmospheric heatwave as a strong event, whereas the `r round(df_Temp |> dplyr::filter(variable == "SST" & t == "2021-09-07") |> dplyr::pull(temp) - df_Temp |> dplyr::filter(variable == "SST" & t == "2021-09-01") |> dplyr::pull(seas),1)` °C anomaly in water temperature classified the marine heatwave as a moderate event.

The Sentinel-2 image from the 6th of September, captured two days after the start of the AHW already revealed brown seagrass leaves in the true-color composition (@fig-S2_comparison C). This contrasts with the reference image from the 1st of September 2021, taken before the heatwave began (@fig-S2_comparison A), where no brown patches can be observed. Before the event, all QPs appeared green on the Sentinel-2 image, with similar reflectance spectra, typical of green seagrass leaves (@fig-S2_comparison A and D). Their reflectance spectra showed a peak at 560 nm (in the green part of the spectra), low values at 665 nm and a high infrared plateau (> 705 nm).
However, on the 6th of September, QPs classified as impacted during the field campaign, showed significant differences in their reflectance spectral shape compared to unimpacted QPs (@fig-S2_comparison C and E). The reflectance spectra of brown seagrass were characterized by the loss of the reflectance peak at 560 nm and a decrease in the infrared plateau, which was replaced by a steadily increasing slope up to 940 nm. The darkening of large areas within the meadow could also be observered in the true color composition (@fig-S2_comparison C)
...
Using Sentinel-2 data and the QPs, SHSI was calculated for green seagrass unaffected by the heatwave (QPs unimpacted @fig-S2_comparison C), showing minimal changes of `r round(df_indices_S2 |> dplyr::filter(Raster == "During" & Target_types == "Unimpacted seagrasses" & Indices == "SDI") |> dplyr::pull(y) *100)` % between the 1st and the 6th of September (@fig-NDVI_GLI_SPC). In contrast, seagrass impacted by the heatwave and turned brown (QPs impacted @fig-S2_comparison C) exhibited significant SHSI changes, showing an increase of `r round(df_indices_S2 |> dplyr::filter(Raster == "During" & Target_types == "Impacted seagrasses" & Indices == "SDI") |> dplyr::pull(y) *100)` % during the heatwave exposure (@fig-NDVI_GLI_SPC). One month after the event, on the 8th of October 2021, the SHSI of unimpacted seagrass had increased by `r round(df_indices_S2 |> dplyr::filter(Raster == "After" & Target_types == "Unimpacted seagrasses" & Indices == "SDI") |> dplyr::pull(y) *100)` % compared to the 1st of September. Regarding impacted seagrass, one month after the event, the SHSI decreased to values comparable to those of unimpacted seagrass. This change reflects an increase of `r round(df_indices_S2 |> dplyr::filter(Raster == "After" & Target_types == "Impacted seagrasses" & Indices == "SDI") |> dplyr::pull(y) *100)` % compared to values recorded on the 1st of September. 
...
#| echo: false
#| warning: false
#| fig-cap: "Changes in the relative Seagrass Heat Shock Index (SHSI) estimated from Sentinel-2, before (1st of September 2021), during (6th of September 2021) and after (8th of October 2021) a heatwave in the seagrass meadow of Quiberon (South Brittany, France). The relative SHSI difference was calulated using the 1st of September as a reference. SHSI was calculated for two categories of Quardrat Points (QPs; @fig-S2_comparison): unimpacted seagrass (green) and impacted seagrass (orange). Points represent the predicted value of the metric using a Generalized Linear Model (GLM), while th error bar repsents the 89% confidence interval. "
#| label: fig-NDVI_GLI_SPC
#| fig-width: 10
#| fig-height: 10
#| out-width: "95%"

knitr::include_graphics("Figs/Low_res/SHSI_S2.png")
```
...
SDI_after2 = (after2_corr$B3+ ((740-560)/(842-560))*(after2_corr$B8-after2_corr$B3)) - after2_corr$B6
...
Using the SHSI (@eq-SDI), we detected large darkened seagrass patches in the meadow at the 6th of September (@fig-Map_darkening_Bathy). A total of `r round(sum(sf_darkseagrass$area_num) * 1e-4, 2)` hectares of seagrass turned brown betwwen the 1st and the 6th of September. The largest brown patch covered nearly `r round(max(sf_darkseagrass$area_num) * 1e-4)` hectares. Overall, `r round((sum(sf_darkseagrass$area_num)/sum(seagrass_extent$area_num))*100)` % of the total seagrass meadow area showed signs of darkening between the 1st and 6th of September 2021. Comparing the spatial distribution of darkened patches with the site’s topography revealed that `r Percent_above_bathy` % of darkened areas were located above a bathymetric level of 3.9 meters (@fig-Map_darkening_Bathy, A and B). One month later, on the 8th of October, previously darkened areas regained their green color (@fig-Map_darkening_Bathy C).

```{r Fig Map_darkening_Bathy}
#| cache: false
#| echo: false
#| warning: false
#| fig-cap: "Sentinel-2 color composition of the seagrass meadow of Quiberon, South Brittany, France, Before (A), During (B) and After (C) the heatwave. Seagrass Heat Shock Index applyed on Sentinel-2 images Before (D), During (E) and After (F) the heatwave."
#| label: fig-Map_darkening_Bathy
#| fig-width: 10
#| fig-height: 6.2
#| out-width: "95%"

knitr::include_graphics("Figs/Low_res/SHSI_S2_map.png")
```

Additionally, seagrass emersion time revealed a clear relationship between the duration of air exposure and seagrass darkening (@fig-GAM_Emersion). During the heatwave, no significant darkening occurred with less than 13 hours of daily exposure. However, above 13 hours, seagrasses began to darken, reaching peak darkening at around 14.5 hours of daily exposure.

```{r Fig GAM_Emersion}
#| cache: false
#| echo: false
#| warning: false
#| fig-cap: "Relative change of the Seagrass Darkening Index (SDI) before and during the heatwave events as a function of the daily emersion time of seagrass. The line represents a Generalized Additive Model (GAM) prediction, and the shaded area indicates the standard error. Shaded points represent raw data, each corresponding to a single pixel of the meadow."
