...
Using Sentinel-2 reflectance values extracted from GCPs before and during the events, radiometric indices were calculated and compared. Column A of @fig-NDVI_GLI_SPC, which represents indices from GCPs classified as green seagrass during the field trip on September 10th (i.e., seagrass not affected by the heatwave events), shows no notable differences before or during the event, regardless of the index. However, Column B of @fig-NDVI_GLI_SPC shows significant differences in the indices of GCPs classified as darkened seagrass during the same field trip (i.e., seagrass affected by the heatwave events).

The NDVI dropped from a median of 0.61 to 0.40, while also becoming less variable (standard deviation decreasing from 0.09 to 0.06) (@fig-NDVI_GLI_SPC B1). Although the seagrass density remained consistent before and during the event, the drop in NDVI affected the retrieved Seagrass Percent Cover (SPC) values (@fig-NDVI_GLI_SPC B2). The SPC decreased from a median of 83% before the event to 48% during the event. A similar trend is observed with the GLI, which dropped from a median of 0.15 before the event to 0.02 after the event (@fig-NDVI_GLI_SPC B3).

Interestingly, the density of seagrass that remained green after the event (median SPC = 65%; @fig-NDVI_GLI_SPC A2) was initially lower than the density of seagrass that darkened (median SPC = 85%; @fig-NDVI_GLI_SPC B2).
...
df_all_indice <- read.csv("Data/Sentinel2/df_all_spectra_GCP.csv") %>% 
  ...
  mutate(date = case_when(Raster == "After" ~ ("2021-09-06"), 
                          T ~ ("2021-09-01")),
         Indices = case_when(Indices == "NDVI" ~ "1 - NDVI",
                             Indices == "SPC" ~ "2 - SPC",
                             Indices == "GLI" ~ "3 - GLI"),
         Target_types = case_when(Target_types == "Bleached_seagrass" ~ "B - GCPs Darkened in the field",
                                  T ~ "A - GCPs Green in the Field")) 
...
plot_indices <- df_all_indice %>% 
  ...
  geom_boxplot(show.legend = F)+
  facet_grid(Indices ~ Target_types, scales = "free_y")+
...
ggsave("Paper/Figs/High_res/Compares_Indices_S2.png",plot_indices, width =7 , height = 10, dpi = 800)
```
...
#| echo: false
#| warning: false
#| fig-cap: "Comparaison of Sentinel-2 based radiometric indices at two different dates, before (2021-09-01, green) and after (2021-09-06, red) the heatwaves events of Quiberons, for the two category of GCPs seen in the filed : Green seagrasses (Column A) and Darkened Seagrasses (Column B). The Normalised Difference Vegetation Index (1-NDVI) has been computed using the @eq-ndvi, the Seagrass Percent Cover (2-SPC) with the @eq-spc and the Green Leaf Index (3-GLI) using @eq-gli,"
...
writeRaster(b, "Data/Litto3D/Litto_1m_merged.tif")
...
contours = terra::as.contour(b_32630_cropped, levels = seq(-2,5,by = 1))
...
write_sf("Output/Bathy_lines.shp")
...
```{r making bathy and darkening index maps}
#| cache: true
#| echo: false
#| eval: false
#| warning: false
...
main_plot <- ggplot() +
  geom_spatraster_rgb(data = RGB_comp,max_col_value = 220 ) +
...
inset_plot <- ggplot() +
  geom_spatraster_rgb(data = RGB_comp,max_col_value = 150 ) +
  geom_sf(data = bathy_line, aes(color = "bathy_color"), linewidth = 0.7) +
