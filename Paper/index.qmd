...
The two cloud-free Sentinel-2 images used in this study are dated 1 September 2021 and 6 September 2021 (@fig-S2_comparison A and C, respectively). The Atmospheric Heat Wave (AHW) started on 4 September and lasted until 7 September, while the Marine Heat Wave (MHW) started on 3 September and ended on 8 September 2021 (@fig-S2_comparison B). The AHW has been classified as a strong event, and the MHW has been classified as moderate. Even though the Sentinel-2 images were captured only a few days after the start of the events, darkening of seagrass can already be observed (@fig-S2_comparison C). The class of Ground Control Points (GCPs) acquired in the field on 10 September corresponds to the locations of dark patches visible in the images from 6 September. Before the event, there were no notable differences in the reflectance of the two classes of GCPs (@fig-S2_comparison D, left). Both spectra show strong absorption at 668 nm (chlorophyll-a), a high infrared plateau, and a reflectance peak in the green region of the spectrum. In contrast, spectra extracted from the 6 September image show clear differences between the two classes of GCPs (@fig-S2_comparison D, right). GCPs corresponding to green seagrass observed in the field maintained a similar spectral shape as before the heatwave events. However, GCPs corresponding to darkened seagrass observed in the field have lost their high reflectance in the green part of the spectrum, and the infrared plateau has decreased significantly, now exhibiting a downward slope.

```{r Fig S2_comparison}
#| cache: false
#| echo: false
#| warning: false
#| fig-cap: "Spectral Comparison of Sentinel-2 Signatures (D) of Ground Control Points (GCPs) Before (A) and After (B) Heatwave Events (C) ; A : RGB color composition of the Sentinel-2 image from September 1, 2021. The points correspond to GCPs collected on September 10, 2021 [@fig-quiberonMap] ; B : Detection of heatwave events based on both Air Temperature and Sea Surface Temperature. The solid line represents the daily average temperature, while the dashed line indicates the 90th percentile of the climatology. When the daily average exceeds the 90th percentile for three consecutive days, the event is classified as a heatwave (colored polygons). The two vertical dashed lines represent the acquisition dates of the two Sentinel-2 images ; C : RGB color composition of the Sentinel-2 image from September 6, 2021. The points correspond to GCPs collected on September 10, 2021 [@fig-quiberonMap] ; D : Average spectral signatures of GCPs where dark and green seagrasses (blue and gold lines, respectively) were present during the field survey. The left plot shows the reflectance of these GCPs before the heatwave impact, while the right plot shows the spectral signature after the heatwaves.The ribbons around the lines represent the standard deviation."
...
df_all_indice <- read.csv("Data/Sentinel2/df_all_spectra_GCP.csv") %>% 
  ...
  dplyr::select(c(filename, Targets_name,Raster,NDVI,GLI,SPC)) %>%
  ...
  mutate(date = case_when(Raster == "After" ~ ("2021-09-06"), 
                          T ~ ("2021-09-01")),
         Targets_name = case_when(Targets_name == "Bleached_seagrass" ~ "GCPs Darkened in the field",
...
plot_indices <- df_all_indice %>% 
  ...
  geom_boxplot(show.legend = F)+
  facet_grid(Indices ~ Targets_name, scales = "free_y")+
...
#| echo: false
#| warning: false
#| fig-cap: "Comparaison of Sentinel-2 based radiometric indices at two different dates, before (2021-09-01) and after (2021-09-06) the heatwaves events of Quiberons, for the two category of GCPs seen in the filed : Green seagrasses and Darkened Seagrasses. SPC stands for Seagrass Percent Covers and it's calculated using the equation of @zoffoli2020sentinel."
#| label: fig-NDVI_GLI_SPC
#| fig-width: 7
#| fig-height: 10
#| out-width: "80%"

knitr::include_graphics("Figs/High_res/Compares_Indices_S2.png")
```
...
stk <- c(NDVI_before,NDVI_after,GLI_before,GLI_after)%>% 
  ...
  as.data.frame() %>% 
  mutate(diff_NDVI = (NDVI_before-NDVI_after),
         diff_GLI = (GLI_before-GLI_after))

ggplot(stk, aes(NDVI_before, diff_GLI))+
  geom_point()+
  xlim(c(0.4,0.8))+
  # ylim(c(0,0.25))+
  geom_smooth(method = "loess", color = "darkred") +
  theme_Bede()+
  ylab("Drop in GLI after the Heatwave")+
  xlab("NDVI before the Heatwave") + 
  theme(axis.text.x = element_text(size = 15),
        axis.text.y = element_text(size = 15),
        axis.title.x = element_text(size = 20),
        axis.title.y = element_text(size = 20))

# Create a data frame with the latitude value
# Use these UTM values fo

NDVI$B8

ggplot()+
  geom_spatraster(data = NDVI, aes(fill = B8))+
  theme_Bede_Map()+
  scale_fill_grass_c(palette = "viridis", limits = c(0.1,0.8))+

  coord_sf(ylim = c(5269073,5271627),
           xlim = c(490212, 491300))
