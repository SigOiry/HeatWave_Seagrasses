...
keywords:
  ...
  - Heatwaves 
abstract:
  This study investigates the impact of marine and atmospheric heatwaves on the pigment composition and reflectance of the intertidal seagrass Nanozostera noltei. Seagrasses are critical to coastal ecosystems, providing habitat, stabilizing sediments, and aiding carbon sequestration. Climate change has increased the frequency and intensity of heatwaves, potentially threatening seagrass health. In this experiment, we exposed N. noltei samples to controlled heatwave conditions and used hyperspectral radiometry to assess changes in spectral signatures over time. Results revealed that heatwaves induce significant declines in seagrass reflectance, particularly in the green and near-infrared regions, linked to pigment degradation. Key vegetation indices, such as the Normalized Difference Vegetation Index (NDVI) and Green Leaf Index (GLI), also displayed marked reductions under heatwave stress, with NDVI values decreasing by up to 34% and GLI by 57%. A novel Seagrass Darkening Index (SDI) was developed to quantify seagrass darkening, showing a strong correlation with heatwave exposure. This research suggests that spectral monitoring can effectively track the early impacts of heatwaves on seagrasses, providing a valuable tool for remote sensing-based habitat assessment. Satellite observations confirmed these findings, showing widespread seagrass darkening during heatwave events in Quiberon, France. This work highlights the need for continuous monitoring of seagrass meadows under the current climate regime, underscoring the potential of remote sensing in capturing rapid environmental changes in intertidal zones.
plain-language-summary: |
  To be written
key-points:
  - Heatwaves significantly reduce seagrass reflectance and pigment composition.
  - Novel Seagrass Darkening Index quantifies stress-induced color changes in seagrasses.
  - Hyperspectral radiometry reveals detailed spectral responses to thermal stress in seagrass.
  - Remote sensing proves effective in monitoring seagrass health during extreme events.
  - Findings highlight rapid impacts of heatwaves on seagrasses.
date: last-modified
citation:
...
To experimentally apply temperatures that closely mimic those experienced by seagrasses in the field, *in situ* temperature sensors were placed at the sampling site in Bourgneuf Bay during August 2024. The daily maximum temperatures recorded by the *in situ* sensors were compared to those measured by the nearest weather station operated by Meteo France (@fig-Temperature_Bourgneuf). On average, *in situ* temperatures were 3 ± 3.2°C higher than those recorded by Meteo France. Additionally, Meteo France temperatures were more stable compared to those from the *in situ* sensor. This stability is explained by the fact that Meteo France temperatures are measured in a sheltered, shaded location . This offset was used to adjust heatwave temperatures measured by Meteo France to reflect the conditions experienced by the seagrasses.

The control chamber was maintained at temperatures representative of the typical seasonal conditions: water temperatures between 18°C and 19°C and air temperatures between 18°C and 23°C, following circadian temperature variability (@fig-Profile left). For the experimental treatment, the air temperature was set to mimic an atmospheric heatwave that occurred over the seagrass meadow of Quiberon, France (47°35'40.0"N, 3°07'30.0"W) from September 2, 2021, to September 6, 2021. On the first day of the experiment, air temperatures in the experimental chamber were set to range from 23°C at night to 35°C during the day, with a daily increase of 1°C. The water temperature in the experimental chamber was similarly adjusted to reflect the heatwave conditions, starting from the normal seasonal range (18°C) and gradually increasing to simulate the rising temperatures experienced during the heatwave (+0.5°C daily). This setup aimed to replicate the thermal stress experienced by the seagrass meadow during the actual heatwave event (@fig-Profile right). The experiment stops only when no changes in the reflectance of the treatment are observed for 2 consecutive days.
...
Based on these evolution of spectral feature of seagrasses exposed to extreme events, a radiometric index of the darkening of seagrass can be made. Darkening of seagrasses is not only characterized by a drop in reflectance in the green regions but also by a significant drop in the red edge part of the electromagnetic spectra (@fig-Exp_Spectral_indices). 
...
Where $R(560)$, $R(740)$ and $R(842)$ are the reflectances at 560, 740 and 842 nm respectively. This Wavelength have been chosen to match the spectral resolution of satellite such as Sentinel-2, making this index usable to map darkening events using satellites. the SDI has been design to be a binary classifiers, where negative values indicates green seagrasses and positive values indicates darkened seagrasses. 
...
Applied to reflectance data collected during the experiment, the SDI indicated distinct differences between the Control and Treatment groups. At the end of the experiment (i.e. when seagrasses in the Treatment group had visibly darkened), the median SDI of the Control group was -0.003, with only 3% of samples displaying a positive SDI value. In contrast, the Treatment group exhibited a median SDI of 0.01, with no samples showing a negative SDI value (@fig-SDI_exp, right).
...
result <- GCP %>%
  ...
  mutate(Raster = case_when(str_detect(Band,"raster1") ~ "Before",
                            T ~ "After"),
         Band = gsub(".*_","",Band))
...
NDVI <- result_cleaned %>% 
  ...
  mutate(NDVI = ((`832`-1000) - (`664`- 1000))/((`832`-1000) + (`664`-1000))) %>% 
...
Using Sentinel-2 data, the spectral indices (NDVI and GLI) and SPC estimated for pixels unaffected by the heatwave showed no significant differences before or during the event (@fig-NDVI_GLI_SPC, left column) In contrast, seagrass darkened by the heatwave exhibited a significant decrease in both index values and SPC (@fig-NDVI_GLI_SPC, right column). In this case, the NDVI dropped by approximately 34%, from a median of 0.61 to 0.40, while also becoming more homogeneous (the standard deviation decreased from 0.09 to 0.06) (@fig-NDVI_GLI_SPC B). Although the heatwave did not affect the observed seagrass density in the field, the drop in NDVI led to an underestimation of satellite-based SPC, from 83 to 48% (@fig-NDVI_GLI_SPC D). The GLI, the index that reflects the greenness of leaves, was the most affected by the event, with an important reduction from a median of 0.15 to 0.02 during the event (@fig-NDVI_GLI_SPC F). Interestingly, when comparing the initial seagrass density of both targets (i.e., seagrass density before the event) pixels affected by the heatwave (median SPC = 65%; \[@fig-NDVI_GLI_SPC C green) were denser than the those remained unchanged (median SPC = 85%; \[@fig-NDVI_GLI_SPC D green).
...
#| echo: false
#| warning: false
#| fig-cap: "Seagrass Darkening Index as a function of the daily emergence time of seagrasses. The line represents a Generalized Linear Model (GLM) prediction, and the shaded area indicates the standard error. Shaded points represent raw data, with each point corresponding to a single pixel."
