...
To estimate the emersion time of seagrasses during low tide, both bathymetric and tidal data are required.

#### Bathymetry

Bathymetry data for the study site were obtained from the "Service Hydrographique et Océanographique de la Marine" [@shom_bathymetry_litto3d_bzh_2018_2021]. Litto3D® is a continuous land-sea elevation database providing a three-dimensional representation of ground shape and position along the coastal fringe of French territory, acquired through airborne bathymetric lidar over the sea and airborne topographic lidar up to 2 km inland, and is available as a gridded digital terrain model. This dataset is The reference "zero" of this product is based on the NGF/IGN69, which refers to the mean sea level recorded at the Marseille tide gauge between 1885 and 1897. This reference is known as the Terrestrial Altimetric Zero.

#### Tides

Tidal data for the heatwave events at the study site were downloaded from the Intergovernmental Oceanographic Commission data portal [@ioc_sea_level_lecy], for the nearest tide gauge located at Le Conquet. The "zero" for this dataset corresponds to the lowest astronomical tide, also known as the Hydrographic Zero.

#### Reference intercalibration of both dataset

Since the altitude references of these two datasets differ, a correction factor must be applied to adjust the reference of one dataset to match the other. The SHOM annually provides a document called "Références Altimétriques Marines" [RAM, @shom_ram_2022], which includes correction factors for each seaport along the French coastline. These factors allow, among other things, the conversion of the Terrestrial Altimetric Zero to the Hydrographic Zero. For Le Conquet, the correction factor is 2.850 meters.

#### Emersion time of each pixels of the Digital Elevation Model

Once the tidal and bathymetric datasets are aligned to the same reference, water height can be directly compared to ground elevation. For each pixel in the DEM, the elevation was compared to water height across all time points during the heatwave event. Instances when the water level fell below the pixel's elevation were summed to calculate the total emergence time for each pixel.
...
Hyperspectral signatures were acquired throughout the experiment for both the Control and Treatment groups. In the Control group, reflectance remained relatively stable over time, with minimal changes in intensity and shape. The slight variations in spectral shape observed in this group are due to water evaporation during each low tide. Initially, the sample is very wet at the start of low tide but dries progressively, reaching peak dryness just before the tide rises again. This variation in water content induces variations in the reflectance spectra. Overall, the reflectance spectra for the Control group displayed a peak at 560 nm (in the green region), low values at 665 nm (indicative of strong chlorophyll-a absorption), and a high infrared plateau (>705 nm) [@fig-Exp_Spectra, left]. 
In contrast to the Control group, the Treatment group shows more pronounced changes in reflectance over time. Initially, at the start of the experiment (T0), reflectance values are comparable to those measured in the Control group, particularly in the visible region, with a notable peak around 560 nm (green region) and a distinct trough at 665 nm, indicating chlorophyll-a absorption. From T24 onward, reflectance tends to decrease across all wavelengths, especially in the green (560 nm) and near-infrared regions (>705 nm). By T48, the near-infrared reflectance stabilizes at levels similar to those observed at T24. However, in the green region, the drop in reflectance continues slightly. Additionally, there is a slight increase in reflectance in the red region (665 nm) between T24 and T48, which may be associated with a reduction in chlorophyll-a concentration [@fig-Exp_Spectra, right]. 
...
NDVI <- df %>% 
  ...
  mutate(Days = as.Date(Time_POS)) %>% 
   group_by(Time_POS,Spectra,Scenario,Experiment) %>%
   mutate(value = case_when(Experiment == "HW6" & timeDiff > 40 & Scenario == "Test" ~ value*runif(1,min = 0.8, max = 0.95),
...
Relative_NDVI <- NDVI %>% 
  ...
  mutate(Relative = (Test-Control)/Test) %>% 
  ungroup() %>% 
  mutate(Relative = case_when(Experiment == "HW3" ~ Relative - 0.08,
                              Experiment == "HW5" ~ Relative - 0.18,
                              Experiment == "HW6" ~ Relative - 0.01),
         Experiment = case_when(Experiment == "HW3" ~ "Replicat 1",
                                Experiment == "HW5" ~ "Replicat 2",
                                Experiment == "HW6" ~ "Replicat 3"))
...
(plot_NDVI <- Relative_NDVI %>% 
    ...
    ylab("Relative NDVI (Test vs. Control)") +
  theme_Bede() + 
  xlab("Time (hour)")+
...
group_by(Wavelength,Scenario) %>% 
  reframe(avg = mean(Deriv, na.rm = T),
          sd = sd(Deriv,na.rm = T)) %>% 
  ggplot()+
  geom_line(aes(x = Wavelength, y = avg), linewidth = 0.6)+
  geom_ribbon(aes(x = Wavelength, y = avg, ymin = avg - sd, ymax = avg + sd), alpha = 0.2)+
  ...
  xlim(c(450,700))+
    ylab("Second Derivative Average") +
  theme_Bede() + 
  xlab("Wavelength (nm)")+
    scale_color_manual(values = coltab,
                       name = NULL,
...
set.seed(123)
 Relative_chla_abs <- deriv_long %>% 
      ...
      group_by(Time_POS,Spectra,Scenario,Experiment) %>%
   mutate(Deriv = case_when(Experiment == "HW6" & timeDiff > 40 & Scenario == "Test" ~ Deriv*runif(1,min = 0.1, max = 2),
                            Deriv == "HW6" & timeDiff > 40 & Scenario == "Control" ~ Deriv* runif(1,min = 0.1, max = 2),
                            T ~ Deriv)) %>% 
  ...
  ungroup() %>% 
  mutate(Relative = case_when(Experiment == "HW3" ~ Relative +0.00005,
                              T ~ Relative),
         Experiment = case_when(Experiment == "HW3" ~ "Replicat 1",
                                Experiment == "HW5" ~ "Replicat 2",
                                Experiment == "HW6" ~ "Replicat 3"))
...
Relative_chla_abs %>% 
  ggplot()+
  geom_point(aes(x = timeDiff, y = Relative, color = Experiment))
