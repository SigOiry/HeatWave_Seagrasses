...
At the start of the experiment (day 1), there was no notable difference in $R''_{665 \, \text{nm}}$, NDVI and GLI between the Treatment and Control groups (@fig-Exp_Spectral_indices A, B & C, Annexe B). During days 2 and 3 the radiometric indices all decreased significantly, with an overall decline of `r abs(round(df_model_hyperspectral |> dplyr::filter(days == 3 & variable == "abs_chla") |> dplyr::pull(y) *100))`%, `r abs(round(df_model_hyperspectral |> dplyr::filter(days == 3 & variable == "NDVI") |> dplyr::pull(y) *100))`% and `r abs(round(df_model_hyperspectral |> dplyr::filter(days == 3 & variable == "GLI") |> dplyr::pull(y) *100))`% for $R''_{665 \, \text{nm}}$, NDVI and GLI. 
...
model_gam_brms <- brm(
  formula = bf(Relative_SDI ~ s(emersion_per_day,k=6)),
  data = stk_all_df_gam_sdi, 
  family = gaussian(), 
  cores = 8, 
  iter = 5000,
  ...
  control = list(adapt_delta = 0.9999),
  save_pars = save_pars(latent = TRUE),
  file = "Output/gam/gam_SHSI_vs_Emersion_5000iter_k6"
  )

plot(model_gam_brms)
summary(model_gam_brms)


                     
NewData_gam_brms<-data.frame(emersion_per_day = seq(from = min(stk_all_df_gam_sdi$emersion_per_day), 
                                                    to = max(stk_all_df_gam_sdi$emersion_per_day), 
                                                    by = 0.01))

InitialData_SHSI_brms<-NewData_gam_brms %>%
  tidybayes::add_linpred_draws(model_gam_brms, ndraws=2000,seed=123)%>% 
  group_by(emersion_per_day)%>% 
  summarise(tidybayes::median_qi(.linpred,.width=c(0.89))) %>% 
  mutate(variable = "Relative_SHSI")
...
(plot_GAM <- ggplot()+
    geom_line(data = InitialData_SHSI_brms, aes(x = emersion_per_day, y = y*100), color = "black",linewidth = 1)+
    geom_ribbon(data = InitialData_SHSI_brms, aes(x = emersion_per_day, y = y*100, ymin = ymin*100, ymax = ymax*100) ,fill = "black", color = NA, alpha = 0.2)+
  ylab("Relative change of SDI (%)")+
  xlab("Emerged time per day (hour)")+
  geom_point(data = stk_all_df_gam_sdi , aes(x = emersion_per_day , y = Relative_SDI*100),alpha = 0.01)+
    geom_hline(yintercept = 0)+
...
summary_gam_SHSI <- summary(model_gam_brms)

# Convert to a clean data frame
table_gam_SHSI <- as.data.frame(summary_gam_SHSI$fixed) %>%
  rownames_to_column(var = "Variable") %>% 
  # mutate(Index = "GLI") %>%
  dplyr::select(
    # Index = Index,
...
Additionally, there was a clear relationship between seagrass emersion time and seagrass darkening (@fig-GAM_Emersion ; @sec-AnnexeC). Seagrass emerged during less than 13 hours a day were not impacted, whereas seagrass emerged during more than 13 hours turned brown with the maximum darkening occurring on seagrass emerged during more than 14.5 hours daily. 
...
This research was funded by the REWRITE (Rewilding European Shorelines and Beyond) project funded by the European Union under Grant Agreement 101081357.
