...
p2 <- ggplot() +
  tidyterra::geom_spatraster_rgb(data = RGB_Low, alpha = 0.5) +
  ...
  theme_Bede_Map() +
  guides(
    fill = guide_legend(order = 2),  # Merge the fill legend
    color = guide_legend(order = 1)  # Merge the color legend
  ) +
  theme(
    ...
    legend.box = "vertical",  # Combine legends into one block
    legend.spacing.y = unit(0.0, 'mm')  # Optional: Change to "right", "bottom", etc., as needed
  )
