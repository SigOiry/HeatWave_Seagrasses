...
Seagrass samples were taken in summer 2024, at low tide, from a *Zostera noltii* (dwarf eelgrass) meadow located in Bourgneuf Bay, France (46°57'32.0"N, 2°10'37.0"W). A metal coring devices was used to sample seagrass from an area of 30x15x5 cm (length x width x depth, respectively), maintaining the sediment structure and avoiding damage to seagrass rhizomes and leaves (@fig-design A). These coring device enabled the collection of sediment samples at a consistent depth, minimizing variability between samples. A total of six samples were collected. Samples including seagrass, sediment, meiofauna, and macrofauna, were placed in plastic trays. Keeping the entire biota allowed for natural interactions between components and reduced stress on the seagrass. Seawater was added to each tray to avoid hydric stress caused by insufficient moisture during transportation (1h drive from the laboratory). Simultaneously, seawater was sampled 4km away from the seagrass sampling site and transported to the lab, where it was filtered using a 0.22 µm nitrocellulose filter to remove suspended particulate matter. The filtered seawater was used in the acclimation tank and the intertidal chambers. The seagrasses were acclimated for one week with a water temperature of 17°C, matching the *in situ* temperature during sampling. During the acclimation phase, seagrasses were maintained under a constant photosynthetically active radiation (PAR) of 150 µmol m⁻² s⁻¹, corresponding to the maximum light intensity provided by the experimental intertidal chambers. This setup ensured that samples were pre-adapted to the same irradiance conditions used during the heatwave experiment, thereby preventing any confounding effects related to light reduction upon transfer to the chambers.
...
To quantify heatwave-induced alterations in seagrass reflectance, we developed the Seagrass Heat Shock Index (SHSI). This metric captures deviations in reflectance around 560 and 740 nm—corresponding to the green-yellow and red-edge spectral regions—where the differences between heat-stressed and non-stressed seagrasses are most pronounced (@fig-Exp_Spectra). 

Because the SHSI is designed to detect changes in spectral shape rather than absolute reflectance values, the reflectance spectrum of each individual sample must first be standardized using a min–max transformation ([@eq-std] ; @Cao2017)

$$
R_{i}^{*}(\lambda) = \frac{R_{i}(\lambda) - min(R_{i})}{max(R_{i})- min(R_{i})}
$$ {#eq-std}

The standardized reflectance values $R^*$ at 560 nm and 740 nm are then compared to a linear baseline defined between two anchor wavelengths: 490 nm (blue region) and 865 nm (near-infrared). The blue anchor (490 nm) was selected because impacted and non-impacted seagrasses exhibit nearly identical standardized reflectance in this region, providing a stable reference point. The near-infrared anchor (865 nm) was chosen as the endpoint because both spectra converge at longer wavelengths, making it an optimal reference for multispectral sensors such as Sentinel-2.

$$ 
\begin{aligned}
\mathrm{SHSI} &= LH(\lambda_{green}) + LH(\lambda_{RE}) \\[6pt]
...
where $R^*$ si the min-max standardised reflectance (@eq-std), $\lambda_1$ = 490 nm, $\lambda_2$ = 865 nm, $\lambda_{green}$ = 560 nm, and $\lambda_{RE}$ = 740 nm. By definition, the SHSI is mathematically constrained between −2 and +2, although these extreme values cannot be reached under natural conditions.
A positive SHSI indicates darkened seagrass, whereas a negative SHSI characterizes green vegetation. These specific wavelengths were selected to match the spectral configuration of current and upcoming multispectral satellite missions (e.g., Sentinel-2, Landsat Next), ensuring broad applicability for remote sensing analyses. Because near-infrared wavelengths are strongly absorbed by water, the SHSI can be reliably computed only for emerged seagrasses during low-tide conditions. Even minimal water coverage markedly attenuates reflectance in the NIR region, rendering the index unsuitable under submerged or partially inundated conditions.
...
#| echo: false
#| warning: false
#| fig-cap: "Computation of the reflectance Seagrass Heat Shock Index (SHSI) for Impacted (A) and Unimpacted (B) seagrass leaves. The orange line represents the reflectance interpolation between 490 and 865 nm. The red vertical dashed lines at 560 and 740 nm represent the SHSI line heights."
#| label: fig-SHSI
#| out-width: "95%"
...
Sea Surface Temperature (SST) data from 1982–2022 over the Quiberon coastal area were downloaded from the Copernicus Marine Data Store (@CMEMS_1). The product provides daily sea surface temperature at a spatial resolution of approximately 0.05° × 0.05° (≈5.6 × 3.8 km at 47° N). An area of `r area_poly_sst_rounded` km² was extracted and analyzed. This area was large enough to minimize missing values caused by cloud cover while remaining small enough to limit the influence of offshore SST stability.
...
All radiometric indices, $R''_{665 \, \text{nm}}$, NDVI, GLI and SHSI changed after the experimental heatwave (@fig-Exp_Spectral_indices ; @sec-AnnexeC).
...
With a maximum deviation of `r round(df_model_hyperspectral |> dplyr::filter(days == 3 & variable == "SHSI") |> dplyr::pull(y) *100)` %, SHSI emerges as the most sensitive index for detecting seagrass darkening. Consequently, only this index was considered for the next steps of the study.
...
#| echo: false
#| warning: false
#| fig-cap: "A - Median of the SHSI across experimental runs, on each day of the experiment. Error structure represent the 89 % confidence interval (Quantiles 0.055 and 0.945 for lower and upper ribbon, respectively). The green line shows values of the Control group while the orange line indicates values of the Treatment group. B - Changes in the SHSI estimated from Sentinel-2, before (1st of September 2021), during (6th of September 2021) and after (8th of October 2021) a HW in the seagrass meadow of Quiberon (South Brittany, France). SHSI was calculated for two categories of Quadrat Points (QPs; @fig-S2_comparison): unimpacted seagrass (green) and impacted seagrass (orange). Points represent the estimated value of the SHSI using a GLM, while the error bar represents the 89% confidence interval."
...
This study explored the dual effect of MHWs and AHWs on spectral reflectance of intertidal seagrasses. Significant changes in the reflectance of seagrasses exposed to HWs were observed both in field measurements and the laboratory experiment. Effects were drops of reflectance around 560 nm and around 740 nm (@fig-Exp_Spectral_indices) and of the second derivative at 665 nm (R’’665), NDVI, and GLI (@fig-Exp_Spectral_indices A, B & C) indexes. By contrast, SHSI showed a marked increase, reaching up to 600% for some samples by day 3 (@fig-Exp_Spectral_indices D), which demonstrated its sensitivity at quantifying seagrass darkening. These changes suggest a progressive reduction in photosynthetic activity as well as potential structural or physiological changes to the leaves, such as degraded pigmentation or altered light absorption. Moreover, we observed a considerable decrease in the reflectance in the NIR region (700 - 1300 nm) in stressed plants, which is mainly related with the destruction of the membranes (chloroplasts, thylakoids, cell walls) due to thermal oxidative stress, and the consequent decrease of light scattering through the leaf surface [@knipling1970reflectance]. The leaf darkening observed in plants subjected to thermal stress was mainly explained by the oxidation of phenolic compounds by the enzyme polyphenol oxidase and with a damage of the photosynthetic apparatus [@allakhverdiev2008heat]. High temperatures destabilize chloroplasts membranes, disrupting photosystem II and impairing recovery of photosynthetic function. As chlorophyll-a degrades, the ratio of pigments shifts, with pigments like carotenoids becoming more prominent [@jones1992leaf; @dascaliuc2007heat]. These processes can eventually result in growth impairment and plant death.
...
## Annexes B - Validation of the SHSI {#sec-AnnexeB}

A field campaign was conducted in the seagrass meadow of the Ria de Aveiro Coastal Lagoon (Portugal) from July 14 to 18, 2025. Two distinct study areas were selected, and within each, a grid of randomly distributed sampling stations was established, covering a total surface of 5.6 ha (@fig-Map_Aveiro). At every station, a photo-quadrat of the meadow was collected. Each quadrat image was subsequently analyzed in the laboratory by independent observers, who classified the meadow condition as either impacted by heat stress (i.e., leaves showing visible discoloration or darkening) or unimpacted (i.e., leaves appearing green and healthy). This set of visual classifications was used as the ground-truth dataset.
...
To complement the in situ observations, a Level-2 Sentinel-2 MSI image acquired on July 15, 2025 was downloaded from the Copernicus Open Access Hub [@copernicus_sentinel2], provided by the European Space Agency (ESA). Level-2 products correspond to atmospherically corrected surface reflectance (Sen2Cor processor; @sen2cor) and are delivered as orthorectified data at 10–20 m spatial resolution.
SHSI was computed using @eq-SDI. 
@fig-Map_Aveiro_SHSI indicates that Site A remained largely unaffected by heat stress, except for a localized patch where SHSI values exceeded 0. Conversely, Site B exhibited widespread signs of thermal impact, with SHSI values above 0 across most of the site.
...
site_bboxes <- pts %>%
  ...
  mutate(bbox = map(data, ~{
    bb <- c(xmin = min(.x$lon), xmax = max(.x$lon),
            ymin = min(.x$lat), ymax = max(.x$lat))
    expand_bbox(bb, dx = 0.001, dy = 0.001)
  }))
...
get_bb_from_pts <- function(pts_site, pad = 0.0008) {
...
p_SHSI_A <-
  ...
  scale_fill_gradientn(colors = pal_cols,
                       ...
                       name = "SHSI") +
  coord_sf(xlim = c(bb_siteA["xmin"], bb_siteA["xmax"]),
...
p_SHSI_B <-
  ...
  scale_fill_gradientn(colors = pal_cols,
                       ...
                       na.value = "transparent",
                       name = "SHSI") +
  coord_sf(xlim = c(bb_siteB["xmin"], bb_siteB["xmax"]),
...
ggsave("paper/Figs/High_res/plot_SHSI_Aveiro.svg",plot_4,width = 10, height = 10)
```
...
#| echo: false
#| warning: false
#| fig-cap: "Seagrass Heat Shock Index over Site A (A) and Site B (B) and Sentinel-2 RGB color composition of Site A (C) and Site B (D) of Ria de Aveiro Coastal Lagoon. "
#| label: fig-Map_Aveiro_SHSI
#| out-width: "95%"

knitr::include_graphics("Figs/High_res/plot_SHSI_Aveiro-01.png")
```

Pixels of SHSI were then categorized as:

Impacted when SHSI > 0

Unimpacted when SHSI ≤ 0
...
tbl <- flextable(cm_df) %>%
  ...
  add_header_row(values = c("", "Truth"), colwidths = c(2, 2)) %>%
...
ft_metrics
```
Overall the SHSI can detected impacted meadow with an accuracy of `r ga[[1]]`, a kappa coeffiscient of `r ga[[2]]` and an F1 score of `r mean(metrics_df[,6][[1]])`. All truly impacted pixels have been correctly identified by the SHSI but 19% of truly Unimpacted pixels have shown positive SHSI values.


## Annexes C - Outputs of GLMM {#sec-AnnexeC}
...
## Annexes D - Outputs of GAM {#sec-AnnexeD}
