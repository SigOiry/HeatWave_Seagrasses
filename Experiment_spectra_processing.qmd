...
df <- df  %>%  
  ...
  mutate(std = (Value_mean-min(Value_mean))/(max(Value_mean)-min(Value_mean))) 
```
...
Water_abs %>% 
  ...
  geom_smooth(aes(group = interaction(Date,scenario)), method = "gam" )+
    geom_smooth( method = "glm" )+
...
spectra <- df %>% 
  ungroup() %>%   dplyr::select(std,ID,Wavelength) %>%
...
deriv_long %>% 
  ...
  ggplot(aes(x = Time_POS, y = abs_ratio, color = scenario))+
  ylab("Hauteur du Pic")+
  geom_point(alpha = 0.3)+
  # geom_boxplot(aes(group = interaction(Date,scenario)))+
  geom_smooth(method = "glm")
...
deriv_long %>% 
  ...
  ggplot()+
  geom_line(aes(x = Wavelength, y = diff, color = scenario))+
  geom_vline(xintercept = 655, color = "red")+
  geom_vline(xintercept = 676, color = "blue")+
