...
Intensity_Max = max_light_intensity
  ...
  for (i in 1:2) {
      ...
      ##### Air Temperature
      df_Air_temp <- data.frame(Time_num = seq(0,24, by = 0.25)) %>%
      ...
      df_Water_temp <- data.frame(Time_num = seq(0,24, by = 0.25)) %>%
        mutate(id = (Time_num-min(Time_num))/(max(Time_num)-min(Time_num))*2*pi,
      ...
      df_light <- data.frame(Time_num = seq(0,23.99, by = 0.25)) %>% 
      ...
      df_profile <- df_profile %>% 
        mutate(Hour = substr(as.character(Time), 12, 16),
               Hour = case_when(Hour == "" ~ "00:00",
                                TRUE ~ Hour),
               Minute_num = case_when(substr(as.character(Hour), 4, 5)=="00" ~ 0,
                                      substr(as.character(Hour), 4, 5)=="15" ~ 25,
                                      substr(as.character(Hour), 4, 5)=="30" ~ 50,
                                      substr(as.character(Hour), 4, 5)=="45" ~ 75),
      ...
      data <- df_profile %>%
        ...
        ungroup() %>% 
        mutate(Temp = case_when(Tide_Status == "High_Tide" ~ Temp_Water,
                                TRUE ~ Temp_Air))
      data$tide_ID=NA
      ...
      Profil <- data %>% 
        mutate(UNIX = as.numeric(Time) %>% as.character(),
               ...
               bin_tide = case_when(Tide_Status == "High_Tide" ~ 1, 
                                    TRUE ~ 0),
               light_prof = case_when(nchar(as.character(round(light_intensity))) == 3 ~ as.character(light_intensity),
...
test <- Make_Profile(min_Air_Temp_control = 23,
                     max_Air_Temp_control = 23,
                    min_Air_Temp_test = 20,
                    max_Air_Temp_test = 35.8,
                    ...
                    low_tide_time = "2024-07-03 16:00",
                    sunrise = 6.25,
                    sunset = 22,
