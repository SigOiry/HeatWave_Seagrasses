...
Intensity_Max = max_light_intensity
  ...
  for (i in 1:2) {
      ...
      ##### Air Temperature
      df_Air_temp <- data.frame(Time_num = seq(0,24, by = 1 / 60)) %>%
      ...
      df_Water_temp <- data.frame(Time_num = seq(0,24, by = 1 / 60)) %>%
        mutate(id = (Time_num-min(Time_num))/(max(Time_num)-min(Time_num))*2*pi,
      ...
      df_light <- data.frame(Time_num = seq(0,23.99, by = 1 / 60)) %>% 
      ...
      while(df_profile[nrow(df_profile),1] != Date_end){
        df_profile[nrow(df_profile)+1,1] <- df_profile[nrow(df_profile),1] + (1*60)
      }
