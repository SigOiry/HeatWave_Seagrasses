       ...
       if(!is.na(High_Tide_Until)){
         ...
         data <- data %>% 
          mutate(Tide_Status = case_when(Time <= low_tide_at ~ "High_Tide",
                                         TRUE ~ Tide_Status),
                 Temp = case_when(Time <= low_tide_at ~ Temp_Water,
                                         TRUE ~ Temp))
...
test <- Make_Profile(min_Air_Temp_control = 18,
                    ...
                    date_start = "2024-07-14 23:00:00",
                    date_end = "2024-07-22 23:00:00",
                    low_tide_time = "2024-07-16 13:00:00",
                    High_Tide_Until = "2024-07-16 09:00:00",
                    ...
                    time_step = 60, 
                    Save_Experiment = F, 
                    Experiment_Name = "HW3_15072024_to_22072024")
...
a <- test$Test_df %>% 
  mutate(Time_chr = as.character(Time),
          day = substr(Time_chr, 1,10)) %>% 
  group_by(day) %>% 
  reframe(`T` = max(Temp_Air)) %>% 
  mutate(date = as.Date(as.POSIXct(day, format = "%Y-%m-%d"))) %>% 
  dplyr::select(date,`T` ) %>% 
  dplyr::filter(date >= min(as.Date(test$Test_df$Time)))

```

```{r COmpare with climatology}

climato <- read.csv("Data/MeteoFrance/10km/dep_85_1870_2024.csv") %>% 
  mutate(date = as.POSIXct(substr(as.character(AAAAMMJJHH),1,8), format = "%Y%m%d",tz = "UTC")) %>%
  dplyr::filter(NOM_USUEL=="L ILE D YEU") %>% 
  dplyr::select(c(date,`T`)) %>% 
  group_by(date) %>% 
  reframe(`T` = max(`T`)) %>% 
  bind_rows(a) %>% 
  rename(temp = `T`,
  ...
  clim <- ts2clm(climato, climatologyPeriod = c(min(climato$t), max(climato$t)))
