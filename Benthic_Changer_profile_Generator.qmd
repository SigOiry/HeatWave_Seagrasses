...
Intensity_Max = max_light_intensity
  ...
  for (i in 1:2) {
      ...
      ##### Air Temperature
      df_Air_temp <- data.frame(Time_num = seq(0,24, by = 2 / 60)) %>%
      ...
      df_Water_temp <- data.frame(Time_num = seq(0,24, by = 2 / 60)) %>%
        mutate(id = (Time_num-min(Time_num))/(max(Time_num)-min(Time_num))*2*pi,
      ...
      df_light <- data.frame(Time_num = seq(0,23.99, by = 2 / 60)) %>% 
      ...
      while(df_profile[nrow(df_profile),1] != Date_end){
        df_profile[nrow(df_profile)+1,1] <- df_profile[nrow(df_profile),1] + (2*60)
      }
...
test <- Make_Profile(min_Air_Temp_control = 23,
                    ...
                    date_start = "2024-07-04 09:00",
                    date_end = "2024-07-05 18:00",
                    low_tide_time = "2024-07-04 16:00",
                    sunrise = 6.25,
                    sunset = 22,
