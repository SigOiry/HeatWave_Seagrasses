...
relative SHSI change with emersion time. SHSI was modelled as function
of emersion time with a basis spline. All model parameters were
estimated with a Bayesian framework using the ``brms'' and ``RStan''
...
response variables were modelled assuming a Gaussian distribution, with
...
Shock Index (SHSI) before and during the heatwave events as a function
of the daily emersion time of seagrass. The line represents a
...
control. In contrast, SHSI shows a marked increase, reaching up to 600\%
for some samples by day 3 (Figure~\ref{fig-Exp_Spectral_indices} E).
This positive trend in SHSI aligns with its design as an index to
