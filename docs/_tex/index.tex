...
\citep{shom_bathymetry_litto3d_bzh_2018_2021}. Litto3D® is a continuous
land-sea elevation database providing a three-dimensional representation
of ground shape and position along the coastal fringe of French
territory, acquired through airborne bathymetric lidar over the sea and
airborne topographic lidar up to 2 km inland, and is available as a
gridded digital terrain model. This dataset is The reference ``zero'' of
...
\paragraph{Emersion time of each pixels of the Digital Elevation
Model}\label{emersion-time-of-each-pixels-of-the-digital-elevation-model}

Once the tidal and bathymetric datasets are aligned to the same
reference, water height can be directly compared to ground elevation.
For each pixel in the DEM, the elevation was compared to water height
across all time points during the heatwave event. Instances when the
water level fell below the pixel's elevation were summed to calculate
the total emergence time for each pixel.
...
remained relatively stable over time, with minimal changes in intensity
and shape. The slight variations in spectral shape observed in this
group are due to water evaporation during each low tide. Initially, the
sample is very wet at the start of low tide but dries progressively,
reaching peak dryness just before the tide rises again. This variation
in water content induces variations in the reflectance spectra. Overall,
the reflectance spectra for the Control group displayed a peak at 560 nm
(in the green region), low values at 665 nm (indicative of strong
chlorophyll-a absorption), and a high infrared plateau (\textgreater705
nm) Figure~\ref{fig-Exp_Spectra}. In contrast to the Control group, the
Treatment group shows more pronounced changes in reflectance over time.
Initially, at the start of the experiment (T0), reflectance values are
comparable to those measured in the Control group, particularly in the
visible region, with a notable peak around 560 nm (green region) and a
distinct trough at 665 nm, indicating chlorophyll-a absorption. From T24
onward, reflectance tends to decrease across all wavelengths, especially
in the green (560 nm) and near-infrared regions (\textgreater705 nm). By
T48, the near-infrared reflectance stabilizes at levels similar to those
observed at T24. However, in the green region, the drop in reflectance
continues slightly. Additionally, there is a slight increase in
reflectance in the red region (665 nm) between T24 and T48, which may be
associated with a reduction in chlorophyll-a concentration
Figure~\ref{fig-Exp_Spectra}.
